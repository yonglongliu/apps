...
var ManuTest = {
  ...
  getCameraIdOnSuccess: function (queue, response) {
      ...
      debug('manu getCameraIdOnSuccess' + response);
  },

  getCameraIdOnError: function (queue, response) {
    debug('manu getCameraIdOnError');
  },
	...
	if (camera.length > 0 ) {
	    ...
	    setTimeout(function () {
	        navigator.mozCameras.getCamera(camera[0])
	            .then(ManuTest.gotCamera.bind(ManuTest.this), ManuTest.gotCameraError.bind(ManuTest.this));
	    },100);
	 }
      }
    debug('manu cameraId ' + cameraId);
   },

  gotCamera: function _gotCamera(params) {
    _cameraObj = params.camera;
    debug('manu gotCamera');
  },
  ...
  handleKeydown: function ut_handleKeydown(evt) {
    ...
    switch (evt.key) {
	 ...
	 dump('target.dataset.name' + target.dataset.name);
         if(target.dataset.name == 'camera' || target.dataset.name == 'camera_front'){
              this.checkCamera();
          }
...
window.addEventListener('beforeunload',ManuTest.uninit.bind(ManuTest));
