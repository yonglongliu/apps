    ...
    if (event && event.socket) {
      ...
      socket.ondata = function(event) {
        ...
        if (event.data.indexOf(CMD_RECORD_RESULT) > -1) {
          ...
          if(!commander_lcd_pic_name || !commander_lcd_screen_value) {
            socket.send('cmd:invaid');
            return;
          }
        ...
        case CMD_POWER_OFF:
          ...
          socket.send('cmd:PowerOff,status:ok');

          RemoteHelper.poweroff((response) => {
          }, () => {
            printLog('CMD_POWER_OFF error');
            socket.send('cmd:PowerOff,status:error');
          });
